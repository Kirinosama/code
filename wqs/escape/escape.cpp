...
inline void add(int x){
	int tx=x/m,ty=x%m;
	if(tx-1>=0 && dis[x-m]>=ans1) merge(x,x-m);
	if(tx+1<n && dis[x+m]>=ans1) merge(x,x+m);
	if(ty-1>=0 && dis[x-1]>=ans1) merge(x,x-1);
	if(ty+1<m && dis[x+1]>=ans1) merge(x,x+1);
}
...
int main(){
	...
	for(int i=0;i<n;i++)
		for(int j=0;j<m;j++){
			...
			dis[P(i,j)]=res;D[P(i,j)].val=res,D[P(i,j)].ID=P(i,j);
		}
	for(int i=0;i<n*m;i++) fa[i]=i;
	sort(D,D+n*m);
	for(int i=0;i<n*m;i++){
		ans1=D[i].val;add(D[i].ID);
		if(find(S)==find(T)){
			cout<<ans1<<' ';
			break;
		}
