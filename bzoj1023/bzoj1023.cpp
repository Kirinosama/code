...
void solve2(int x,int f){
	...
	cnt=0;dfs(f,f);
	if(cnt>1){
		for(int i=1;i<=cnt;i++) a[i+cnt+1]=a[i];
		a[cnt+1]=bcc_cnt+1,dp[bcc_cnt+1]=-INF;
		int head=1,tail=0;
		for(int i=2;i<=1+(cnt+1)/2;i++){
			while(head<=tail && dp[a[i]]+i>=Q[tail].second) tail--;
			Q[++tail]=ii(i,dp[a[i]]+i);
		}
		for(int i=1;i<=cnt;i++){
			while(Q[head].first<=i) head++;
			ans=max(ans,Q[head].second+dp[a[i]]-i);
			int pos=i+(cnt+1)/2+1;
			while(head<=tail && dp[a[pos]]+pos>=Q[tail].second) tail--;
			Q[++tail]=ii(pos,dp[a[pos]]+pos);
		}
	}
	for(int i=last[x];i;i=ne[i])
